...
class PurchaseHistoryComponent extends StatefulWidget {
  ...
  @override
  _PurchaseHistoryComponentState createState() =>
      _PurchaseHistoryComponentState();
}
...
class _PurchaseHistoryComponentState extends State<PurchaseHistoryComponent> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: PurchaseHistoryDB.getPurchaseHistoryForUser(
              StaticData.currentUser.userID)
          .asStream(),
      builder: (context, snap) {
        if (snap.connectionState == ConnectionState.waiting) {
          ...
          return Container(
              ...
              child: ListView(
                  children:
                      historyItems.length == 0 ? [Container()] : historyItems));
        }
      },
    );
  }
  ...
  Widget _getHistoryElement(BuildContext context, PurchaseHistoryItem item) {
    double elementHeight = (widget.height * 0.37 > widget.maxElementHeight
        ? widget.maxElementHeight
        : widget.height * 0.37);
