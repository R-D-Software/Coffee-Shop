...
class CartListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      child: Card(
        ...
        elevation: 0,
        child: InkWell(
          onTap: () => Navigator.of(context).pushNamed("/main/itemview/${item.itemType}", arguments: {"itemID": item.documentID}),
          child: Stack(
            children: <Widget>[
              _getInfoRow(context),
              _buildCirclePicture(),
  ...
  Widget _getInfoRow(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(border: Border.all(color: Colors.white, width: 2), color: Color.fromRGBO(189, 150, 150, 1), borderRadius: BorderRadius.all(Radius.circular(height / 2))),
    );
  }
  ...
  Widget _getIcons(BuildContext context) {
    ...
    return sugarIcons.isEmpty
        ? StrokedText(text: "sugar free", color: Theme.of(context).primaryColor, size: 15)
        : Row(
